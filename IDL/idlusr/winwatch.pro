...
;       R. Sterner, 2014 Jun 30 --- Handled COLOR=clr on SET=s_in call.
;       R. Sterner, 2014 Jul 15 --- Handled deleted windows (winwatch_sync).
;
...
;-------------------------------------------------------------
    ;===============================================================
    ;  winwatch_sync = Sync internal state to actual watch windows
    ;===============================================================
    pro winwatch_sync

	common winwatch_com, flag, win_in, win_out, mag, $
	  x1,y1, dx,dy, sv_cnt, sv_nam, oclr

    for i=0, 9 do begin                     ; Loop over possible windows.

      while flag[i] eq 1 do begin           ; If flagged as in use ...

        if win_open(win_out[i]) then break  ; Good window, nothing to do.

        for j=i+1, 9 do begin               ; Bad window.
          flag[j-1]    = flag[j]            ; Shift all values down
          win_in[j-1]  = win_in[j]          ; to overwrite this window
          win_out[j-1] = win_out[j]         ; which is no longer in use.
          mag[j-1]     = mag[j]
          x1[j-1]      = x1[j]
          y1[j-1]      = y1[j]
          dx[j-1]      = dx[j]
    ...
    ;===============================================================
    ;  winwatch = Main winwatch routine
    ;===============================================================
	...
	;  oclr = Watch window outline color.
	;------------------------------------------------------------
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   COLOR=clr Outline color (remembered on next call).'
	  ...
	  print,'   SUBWIN=subwin If used with ARRAY=arr then return a'
	  print,'     structure with the subarrays for each watch window.'
	...
	;------------------------------------------
    ;  Sync internal state to actual windows
	;------------------------------------------
    winwatch_sync
	...
	if n_elements(s_in) ne 0 then begin
      ...
      if n_elements(clr) ne 0 then oclr=clr else oclr=!p.color
	  return
	endif
	...
	if keyword_set(init) then begin
	  ...
	  for j=0,n_elements(in0)-1 do begin	; Loop over requested windows.
        ...
        if in gt 9 then begin
          in = 9
          print,' Only 10 windows allowed, replacing last.'
        endif
	    ...
	    if not keyword_set(nooutline) then begin
	      if n_elements(clr) eq 0 then clr=oclr
	      oclr = clr
	...
	for j=0,n_elements(in0)-1 do begin
	  ...
	  if n_elements(array) ne 0 then begin
	    ...
	    ;---  Return subimages if requested  ---
	    if arg_present(subwin) then begin
	      if j eq 0 then begin			; Start structure.
	        subwin = {win0:sub}
	      endif else begin				; Add next win to strct.
	        tag = 'win'+strtrim(j,2)
		subwin = create_struct(subwin,tag,sub)
	      endelse
	    endif
	  endif
	  ;---  Save watch window  ---
