...
;           double any backslashes (for stregex).  Also the $ may be
;           a problem in csh or tcsh scripts (try +string(36B)).
...
;       R. Sterner, 2012 Jan 09 --- Made FRONT=string error less confusing.
;       R. Sterner, 2012 Jan 09 --- For FRONT=string multiple matches use first.
;
...
;-------------------------------------------------------------
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'     double any backslashes (for stregex).  Also the $ may be'
          print,'     a problem in csh or tcsh scripts (try +string(36B)).'
...
;x	      if min(n) ne 0 then begin
;x	        print,' Error in path: could not move to front.'
;x	        print,'   String ambiguous: '+front
;x	        return
;x	      endif else begin
;x	        print,' Warning in path: target repeated in path.'
;x	        return
;x	      endelse
	      print,' Warning in path: target string repeated in path: '+front
              more,n+1,'  ',new[n]
;x	      return
	      print,'     Moving first one to front.'
	    endif
	    n = n[0]				; Index # to move.
	  endelse
	  ;-----  Move it  -------------
