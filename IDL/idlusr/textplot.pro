...
;-------------------------------------------------------------
	pro textplot, x, y, txt, charsize=csz, orient=ang, alignment=jst, $
	  ...
	  noplot=noplot, offset=offset, anti=anti, noclip=noclip, $
	  txdev=txdev, tydev=tydev, dx=dxout, dy=dyout, dm=dmout
...
hlp:	  print,' Plot text with 2-d alignment.  Also return bounding box.'
	  ...
	  print,'   TXDEV=tx, TYDEV=ty Returned lower left corner of plotted'
	  print,'     text in device coordinates.'
	  print,'   DX=dx, DY=dy  Returned width and height of text in pixels.'
	  print,'   DM=dm  returned width of one character in pixel.'
	  print,'     These 5 keywords are useful in building a plot legend.'
...
3:	begin
	...
	dxout = dx			; Text width (pixels).
	dyout = dy			; Text height (pixels).
	dmout = dx_m			; One character width (pixels).
	...
	if not keyword_set(noplot) then begin
	  if keyword_set(anti) then begin
	    aatext,txdev,tydev,txt,chars=csz,orient=ang,/dev, $
	      charthick=bold, color=color
	  endif else begin
	    xyoutb,txdev,tydev,txt,chars=csz,orient=ang,/dev, $
