...
;-------------------------------------------------------------
	pro maprings, dist, units=units, lng=lng,lat=lat, $
	  color=clr, thickness=thk, linestyle=sty, center=center, $
          psym=psym, symsiz=symsiz, help=hlp
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'     Last 3 items may have a value for each value in dist.'
          print,'   /CENTER mark circle centers (default is do not).'
          print,'   PSYM=psym Circle center symbol (def=3).'
          print,'   SYMSIZ=symsiz Circle center symbol size (def=1).'
	  return
	endif
	...
	if n_elements(symsiz) eq 0 then symsiz=1
	      ...
	      plots, x, y, color=clr[j<lst_c], thick=thk[j<lst_t], $
	        linestyle=sty[j<lst_s]
              if keyword_set(center) then begin
                plots,lng[i],lat[i],psym=psym,symsiz=symsiz,color=clr[j<lst_c]
              endif
	    endfor
	  endfor
	  return
