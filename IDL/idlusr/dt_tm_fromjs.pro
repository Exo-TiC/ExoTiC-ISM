...
;-------------------------------------------------------------
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'   dt = date/time string.      out'
          print,'     Null string if js is NaN.'
	  print,' Keywords: '
	  ...
	  return, -1
        ...
        ;---  Find where js is not Nan  ---
        ingood = where(finite(js,/nan) eq 0,ngood)

        ;---  Set up output array  ---
        njs = n_elements(js)                    ; Number of times to convert.
        out = strarr(njs)                       ; NaN gives a null string.
        if ngood eq 0 then return, out          ; No valid JS.

	js2ymds, js[ingood], year, mm, dd, ss   ; Process just the good JS.
	jd = ymd2jd(year,mm,dd)
	fm = "Y$ n$ d$ h$:m$:s$ w$"
	if keyword_set(fr) then begin
	  fm = "Y$ n$ d$ h$:m$:s$f$ w$"
	  if n_elements(den) gt 0 then fm="Y$ n$ d$ h$:m$:s$ f$ w$"
	endif
	if n_elements(form) ne 0 then fm = form
	if n_elements(dec) eq 0 then dec=3
	out[ingood] = dt_tm_mak(jd, ss, form=fm,dec=dec,den=den,ddec=ddec, numbers=st)
        if njs eq 1 then out=out[0]             ; Return a scalar for 1 element.
	return, out
