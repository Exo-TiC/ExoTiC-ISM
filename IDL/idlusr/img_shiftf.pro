...
;       IMG_SHIFTF
; PURPOSE:
;       Shifts a 2-D or 3-D image by a fractional number of pixels.
; CATEGORY:
; CALLING SEQUENCE:
;       out = img_shiftf(in,dx,dy)
; INPUTS:
;       in = Input image.    in
;       dx = shift in x      in
;       dy = shift in y.     in
...
;-
;-------------------------------------------------------------
	function img_shiftf, img, dx, dy, error=err, help=hlp
 
	if (n_params(0) lt 2) or keyword_set(hlp) then begin
	  print,' Shifts a 2-D or 3-D image by a fractional number of pixels.'
	  print,' out = img_shiftf(in,dx,dy)'
	...
	case typ of
1:	begin
	  r = shiftf(reform(img[0,*,*]),dx,dy)
	  g = shiftf(reform(img[1,*,*]),dx,dy)
	  b = shiftf(reform(img[2,*,*]),dx,dy)
	  out[0,*,*] = r
	  out[1,*,*] = g
	  out[2,*,*] = b
	end
