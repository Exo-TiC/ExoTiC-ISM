...
;       BOX2B
; PURPOSE:
;       Two button interactive box, can return a position box.
; CATEGORY:
; CALLING SEQUENCE:
;       box2b, x1, x2, y1, y2
; INPUTS:
...
;       R. Sterner, 2012 Apr 11 --- Now returns a position box.
;       R. Sterner, 2012 Apr 11 --- Fixed plotted box, was off by 1.
;       R. Sterner, 2012 Apr 20 --- Added DX=dx, DY=dy.
;
...
;-------------------------------------------------------------
	pro box2b, x10, x20, y10, y20, status=stat, help=hlp, $
	  ...
	  info=info, lock=lock, ch_flag=ch_flag, $
          posbox=posbox, device=dev, dx=dx, dy=dy
 
	if keyword_set(hlp) then begin
	  print,' Two button interactive box, can return a position box.'
	  ...
	  print,' Keywords:'
          print,'   POSBOX=posbox Return a position box (normalized by default).'
          print,'     Can be used in POSITION=posbox in a PLOT call.'
          print,'   /DEVICE means return position box in device coordinates.'
          print,'   DX=dx (x2-x1+1).'
          print,'   DY=dy (y2-y1+1).'
...
;	img = tvrd()			; Grab copy of image.
	...
	if min([x10,x20,y10,y20]) ge 0 then begin
	  ...
	  tvbox,x1,y1,x2-x1+1,y2-y1+1,-2, $     ; Plot entry box.
...
loop:
	...
	;-------  Exit box routine  ------------
	  ...
	  if exit ne -1 then begin
	    tvbox,x1,y1,x2-x1+1,y2-y1+1,-1,xmode=xmode  ; Erase box and exit.
	    x10=x1 & x20=x2 & y10=y1 & y20=y2	; Return box.
            dx=x2-x1+1 & dy=y2-y1+1             ; Return box size.
	    ...
	    if keyword_set(stat) then widget_control,wid,/dest
            ;---  Return position box  ---
            if arg_present(posbox) then begin
              if keyword_set(dev) then begin    ; Device position box.
                posbox = [x1,y1,x2,y2]
              endif else begin                  ; Normalized position box.
                nx = float(!d.x_size-1)
                ny = float(!d.y_size-1)
                posbox = [x1/nx,y1/ny,x2/nx,y2/ny]
              endelse
            endif
	    return
	  endif
...
;	  tv, img
	  ...
	  tvbox,x1,y1,x2-x1+1,y2-y1+1,-2, $	; Plot box again.
	    noerase=noerase, xmode=xmode
	  noerase = 0
;	  tvcrs, xcl, ycl
	  tvcrs, (x1+x2)/2, (y1+y2)/2		; Set cursor to midbox.
...
4:	  begin
	  ...
	  endcase
	    ...
	    tvbox,x1,y1,x2-x1+1,y2-y1+1,-2, $		; Plot new box.
	      noerase=noerase, xmode=xmode
...
4:	  begin
	  ...
	  endcase
	    ...
	    tvbox,x1,y1,x2-x1+1,y2-y1+1,-2,xmode=xmode  ; Plot new box.
	    if keyword_set(stat) then begin	; Update status.
	      box2b_stat, x1, x2, y1, y2, nid=nid, yrev=yrev, fact=fact
	    endif
	    if !mouse.button eq 0 then begin
