...
;-------------------------------------------------------------
	pro text_block, out, quiet=quiet, widget=widget, $
	  wait=wait, help=hlp, group_leader=grp
...
help:	  print,' Print or return a block of inline text (lines starts with ;).'
	  ...
	  print,'   GROUP_LEADER=grp Assign a group leader widget ID'
	  print,'     to the text display widget.  When the leader widget is '
	  print,'     destroyed the text display widget will also be destroyed.'
	...
	if keyword_set(widget) then begin
	  xhelp, out, exit_text='OK', /bottom, wait=wait, group=grp
	  return
	endif
