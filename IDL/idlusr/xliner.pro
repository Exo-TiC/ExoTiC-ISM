...
;       R. Sterner, 2010 Jul 21 --- Now can add web pages for routines.
;       R. Sterner, 2010 Jul 26 --- Now can have general text files.  Add text
;         file name to alph.one (without .txt) and have T after the :.  Example:
;         Notes_on_spawning_web_browser_from_IDL: T.
;
...
;	  tmp = getwrd(txt[line],del=':')	; Pick off lib and routine.
	  tmp = getwrd(txt0,del=':')	        ; Pick off lib and routine.
	  lib = getwrd(tmp)
	  rout = getwrd(tmp,1)
	  typ = strmid(getwrd(txt0,del=':',1,/notrim),0,1)  ; Pick off type: P, F, or T.
          if strupcase(typ) eq 'T' then fext='.txt' else fext='.pro'
	  ...
	  dir = s.base[w[0]]
	  f = filename(dir,rout+fext,/nosym)	; Full path to routine.
;	  f = filename(dir,rout+'.pro',/nosym)	; Full path to routine.
	  t = getfile(f)			; Read it in.
	  w1 = where(t eq ';+', c1)		; Look for header.
	  w2 = where(t eq ';-', c2)

          ;---  Try to add a web button  ---
          ;  Look in library subdirectory named web for
          ;  the web page named routine.html, which may be
          ;  in web or in web/routine/.  If not found display an
          ;  error page.  The web browser command to spawn is from
          ;  the file xliner_config.txt in the library directory
          ;  (it was read in the main program section).
          if tag_test(s.scon,'no_config') eq 0 then begin
            wbase = filename(dir,'web',/nosym)
            wld1 = filename(wbase,rout+'.html',/nosym) ; Possible wild cards.
            wld2 = filename(filename(wbase,rout,/nosym),rout+'.html',/nosym)
            cmdlab = 'Web page for '+rout
            wfile = file_search(wld1,count=c)
            if c eq 0 then wfile=file_search(wld2,count=c)
            if c eq 0 then begin
              wfile=filename(wbase,'no_such_file.html',/nosym)
;              wfile=filename('web','no_such_file.html',/nosym)
              cmdlab = 'How to make a web page for '+rout
            endif
            cmd = s.scon.web_command
            rep_txtmarks,cmd,{page:wfile}
          endif else begin
            cmd = ''
          endelse
	  ...
	  if (c1 ne 0) and (c2 ne 0) then begin	; Display just the help text.
	    xhelp,t[w1[0]:w2[0]],exit='Dismiss',save=save_help,$
	      text2=t,t2lab='Source code',command=cmd,comlabel=cmdlab
	  endif else begin			; Display complete routine.
	    xhelp,t,exit='Dismiss',save=save_help,command=cmd,comlabel=cmdlab
	  endelse
	  return
	endif
...
;  Main routine
;
;  This routine looks for a file named alph.one (one line
;  descriptions if each routine) in each library on the IDL
;  path.  If found it is searched for the given keywords and
;  all lines from all alph.one files that match are displayed.
;  Each alph.one extry is the routine name, colon, type,
;  one line description, for example:
;  list_doy:P List day of year for current and nearby dates.
;  Allowed types are P=procedure, F=function, T=text file.
;  May also have web pages for any routine.  May add text
;  files to the alph.one file, for example:
;  Notes_on_spawning_web_browser_from_IDL:T
;  If the given keywords match items in the file name that
;  text file will be displayed.  May also have a web page for
;  a text file.
;-------------------------------------------------------------
 
	pro xliner, help=hlp, addlibs=addlibs, addweb=addweb
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'     libraries that use the standard front-end documentation.'
          print,'   /ADDWEB displays info on how to add a web page for a'
          print,'     routine in the library.  This page may be displayed'
          print,'     from xliner.'
	...
	;====  Display information on adding a web page for a routine  =====
	if keyword_set(addweb) then begin
	  file = filename(dir,'xliner_addweb.txt',/nosym)
	  t = getfile(file,err=err)
	  if err ne 0 then begin
	    xmess,['Could not open:',file]
	    return
	  endif
	  xhelp,t, save='xliner_addweb.txt'
	  return
	endif
 
	;====  Find all alph.one files on path  =======
	path,path=list			; All path directories.
	...
	;----  Check for user defined alph.one files  ------
	home = getenv('HOME')
...
;	  noff = 0				; # of libs to turn off.
	...
	id_results = widget_text(top,xsize=90,ysize=15, /scroll,$
	  /all_events,uval='RESULTS',val=txt)
        ...
        ;----  Read xliner configuration file if any  -----
        cfile = filename(dir,'xliner_config.txt',/nosym)  ; xliner config file.
        scon = getfile_tags(cfile,err=err)                ; Try to read it.
        if err ne 0 then scon={no_config:0}               ; If none set flag.
