...
; KEYWORD PARAMETERS:
;       Keywords:
;         DIGITS=d  Number of digits in result (up to 30).
...
;-------------------------------------------------------------
	...
	if (n_params(0) lt 3) or keyword_set(hlp) then begin
	  ...
	  print,' Keywords:'
	  print,'   DIGITS=d  Number of digits in result (up to 30).'
	  ...
	  print,'   generating file names with embedded numbers.'
          print,'   Numbers that need more digits will have them.'
	  return, -1
	endif
 
	if n_elements(digits) eq 0 then return,strtrim(makei(lo,hi,st),2)
	d = strtrim(digits,2)
	fmt = '(I30.'+d+')'
