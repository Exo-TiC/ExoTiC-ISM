...
;       R. Sterner, 1999 Jun 7 written as putbits.pro.
;       R. Sterner, 2011 Aug 25 --- Renamed to bit_put.pro.
;
...
;-
;-------------------------------------------------------------
        function bit_put, src0, tar, start, num, help=hlp
 
        if (n_params(0) lt 4) or keyword_set(hlp) then begin
          print,' Insert specified bits in a target.'
          print,' out = bit_put(src, tar, s, n)'
	...
	t = datatype(src0,integer_bits=bits)
        if bits le 0 then begin
          print,' Error in bit_put: Source must be an integer.'
          return,-1
        endif
	if bits lt num then begin
	  print,' Error in bit_put: Requested '+strtrim(num,2)+ $
	...
	t = datatype(tar,integer_bits=bits)
        if bits le 0 then begin
          print,' Error in bit_put: Target must be an integer.'
          return,-1
        endif
	if (start+num) ge bits then begin
	  print,' Error in bit_put: '+$
...
else:	stop,' Stopped in bit_put: internal error.'
	endcase
