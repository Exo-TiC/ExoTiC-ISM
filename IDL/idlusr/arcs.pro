...
;-------------------------------------------------------------
	...
	pro arcs, r, a1, a2, xx, yy, help=hlp,$
	  ...
	  xout=xout, yout=yout, pen=pen, noplot=noplot, noclose=noclose
	...
	if (np lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'   /NOCLOSE Do not close the returned curve (x,y,p).'
	  print,'   /NOPLOT do not plot the arcs.'
	  print,' Note: all parameters may be scalars or arrays.'
	  return
	endif
 
	;---  For /device the default is /noclip  ---
	...
	if arg_present(xout) then begin
	  ...
	  xout = [xout,xout[0]]		; Jump back to start point.
	  yout = [yout,yout[0]]
	  pen = [pen,0]
	endif
