...
;-
;-------------------------------------------------------------
	pro ell_rb2ll, lon1, lat1, dst, azi1, lon2, lat2, azi2, help=hlp
 
	if (n_params(0) lt 6) or keyword_set(hlp) then begin
	  print,' From range and bearing on an ellipsoid compute a point.'
	  print,' ell_rb2ll, lon1, lat1, dist, azi1, lon2, lat2, azi2'
	  print,'   lon1,lat1 = Starting point geodetic long and lat (deg). in'
	  ...
	  print,'   lon2,lat2 = End point geodetic long and lat (deg).      out'
	...
	glon1 = lon1*dtor               ; Convert to radians.
	glat1 = lat1*dtor
	faz = azi1*dtor
	...
	baz = 0.D0*faz		        ; Want as many baz az faz.
        ; FORTRAN was: if cf ne 0 then baz=atan(tu,cf)*2
