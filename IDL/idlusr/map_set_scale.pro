...
;-------------------------------------------------------------
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'     MAG=2 If displayed in a window twice as big as original.'
          print,'   /NOCLIP Do not do map specific clipping.'
          print,'     Force clip=0 for map_set.  May be needed to plot a'
          print,'     Lat/Lon grid in some cases.'
	...
	if n_elements(info) eq 0 then begin	; Info array not given.
	  if n_elements(t0) eq 0 then begin	; Image not given.
	    ...
	    wintxt = 'image'
	    if n_elements(mag) gt 0 then begin	; Deal with mag factor.
	      d_x_size = mag*d_x_size
	      d_y_size = mag*d_y_size
	    endif
	  endelse
	...
	;	Deal with new CLIP=
	;=============================================================
	if keyword_set(noclip) then flag_clip=1
