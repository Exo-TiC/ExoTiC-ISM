...
;	  txt = [strmid(txt,0,39),strmid(txt,39,39)]	; 2 columns to 1.
	  txt = [strmid(txt,0,41),strmid(txt,41)]	; 2 columns to 1.
	endelse
	...
	tprint,buff=buf,/init				; Start output text.
 
	;-----  Loop through used windows getting values  ------
	tprint,buff=buf,' Windows in use, sizes, & lower left corner (from screen llc):'
	for i=0,n-1 do begin			; Loop through windows in use.
	  ...
	  if pix[in] then begin			; Window is a pixmap.
	    tprint,buff=buf,intxt+', xsize='+xtxt+', ysize='+$
	      ytxt+', /pixmap  '+txt
	  endif else begin
