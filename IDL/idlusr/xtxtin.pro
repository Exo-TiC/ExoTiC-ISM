...
;         /TEXT_COUNT Give Text Count button to count words & chars.
...
;       R. Sterner, 2012 Aug 03 --- Added /TEXT_COUNT
;       R. Sterner, 2012 Aug 06 --- Added more text stats.
;
...
;-------------------------------------------------------------
	...
	;------  Text count button  ------------
	if name eq 'TEXT_COUNT' then begin
	  widget_control, m.idtxt, get_val=val
          nlines = n_elements(val)
          w = where(strcompress(val,/rem) eq '',nblnk)
          mxlen = max(strlen(val))
          nwds = fix(total(nwrds(val)))
          nchr = long(total(strlen(val)))
          wait,0.1
          txt = ['For the entered text:',' ', $
                 'There '+plural(nlines,'is','are')+' '+strtrim(nlines,2)+ $
                   ' line'+plural(nlines), $
                 strtrim(nblnk,2)+' blank line'+plural(nblnk), $
                 'The longest line has '+strtrim(mxlen,2)+' character'+plural(mxlen), $
                 'There are '+strtrim(nwds,2)+' word'+plural(nwds), $
                 'and '+strtrim(nchr,2)+' character'+plural(nchr)+'.']
          xmess,txt,/wait,/left
;          xmess,txt
;          xmess,strtrim(nwds,2)+' words, '+strtrim(nchr,2)+' characters',/wait
	  return
	endif
...
;=====================================================================
	...
	pro xtxtin, out, title=title, default=def, xsize=xsize, $
	  ...
	  group_leader=grp, text_count=text_count
	...
	if keyword_set(hlp) then begin
	  ...
	  print,"     If YSIZE used then must exit with 'Accept Entry' button."
          print,'   /TEXT_COUNT Give Text Count button to count words & chars.'
	...
	b = widget_button(but, val=cancel_txt,uval='CANCEL')
        ...
        ;---------  Text count button  -----------
        if keyword_set(text_count) then begin
          b = widget_button(but, val='Text count',uval='TEXT_COUNT')
        endif
