;----------------------------------------------------------------------------
;  xpar_parse2.pro = Parse xpar text.
...
;  R. Sterner, 2007 Jun 11 --- Added indices of each item actually used.
;  R. Sterner, 2007 Jun 13 --- Made sure indices used variables are defined.
;
...
;----------------------------------------------------------------------------
;  CURRENT STATUS:
;    06dec18: Appears to be returning the correct tab indices for
;	each item in the returned structure.
;----------------------------------------------------------------------------
;  TO BE CHECKED:
;    Make sure frame rows and columns are right.
;    Frames will start at 0 for each tab.  Size of frame arrays is
;    all frames on all tabs (for each item).  So if there are 2 par_frames
;    on tab 0 (frame 0, and frame 1), and 1 par frame on tab 1, then the
;    par frame arrays will have 3 elements.
;----------------------------------------------------------------------------
;  Notes:  May have multiple frames on the same tab for each type of item
;    (parameters, flags, colors, lists).  The frame numbers are independent
;    for each tab.  That is, tab 0 may have par_frame 0 and tab 1 may also
;    have par_frame 0.
;----------------------------------------------------------------------------

	pro xpar_parse2, t0, s, error=err
	...
	;  Find Tabs
	;
	;  tabnum array will have -1 outside any
	;  tab:/endtab pairs and tab indice
	;  inside each pair.
	;----------------------------------------
	strfind,t,/q,'tab:',count=clo,index=lo
	strfind,t,/q,'endtab',count=chi,index=hi
	if clo ne chi then begin
	  print,' Error in xpar_parse: Number of tab:/endtab lines must match.'
	  print,' tab: lines:'
	  more,t[lo],lines=100
	  print,' endtab lines:'
	  more,t[hi],lines=100
	...
	if clo eq 0 then begin	; No tabs.
	  n_tabs = 0
	  tab_nam = ''
	  tabnum = 0
	endif else begin	; Tab found.
	  tb = [transpose(lo),transpose(hi)]
	  dt = tb[1:*]-tb[0:*]
	  if min(dt) lt 0 then begin
	    print,' Error in xpar_parse: tab:/endtab ranges must not overlap.'
	    print,' Line numbers for:'
	    print,'          tab:        endtab'
	    print,tb
	    err = 1
	    return
	  ...
	  n_tabs = clo
	  tab_nam = strtrim(strmid(t[lo],4),2)

	  tabnum = intarr(n_elements(t)) - 1		; Init to -1.
	  for i=0,clo-1 do tabnum[lo[i]:hi[i]]=i ; Set each line to a tab index.

	  strfind,t,/q,'tab:',/inverse,index=w,count=n	; Drop tab: lines.
	  if n eq 0 then begin				; Any lines left?
	    err = 1
	    print,' Error in xpar: Incomplete parameter text.'
	    return
	  endif
	  if n gt 0 then begin				; Lines left.
	    t = t[w]					; Grab only them
	    tabnum = tabnum[w]				; and tabnums.
	  endif

	  strfind,t,/q,'endtab',/inverse,index=w,count=n ; Drop endtab lines.
	  if n eq 0 then begin				; Any lines left?
	    err = 1
	    print,' Error in xpar: Incomplete parameter text.'
	    return
	...
	if n gt 0 then begin				; Lines left.
	  t = t[w]					; Grab only them
	  if n_tabs gt 0 then tabnum = tabnum[w]	; and tabnums.
	endif
...
;	n_sld = n_sld < n_par				; Limit to n_par sldrs.
	...
	if n_par_frm gt 0 then begin			; If frame lines then
	  par_frm_tab = tabnum[ind]			; Tab index for frame.
	  ...
	  par_frm_tab = 0				; Tab index for frame.
	  n_par_frm = 1
	...
	if n_par gt 0 then begin			; If par lines then
	  par_tab = tabnum[ind]				; Tab index for par.
...
;	  uniq_tagval,par_nam,par_unam, $		; Unique indices.
;	    /first,used=par_usd
;	  par_ind = indgen(n_par)			; Par indices.
	...
	if n_flg_frm gt 0 then begin			; If frame lines then
	  flg_frm_tab = tabnum[ind]			; Tab index for frame.
	...
	if n_flg0 gt 0 then begin		; If flag lines then ...
	  flg_tab0 = tabnum[ind]		; Tab index for flag lines.
	  ...
	  for i=0, n_flg0-1 do begin		; Loop through flag lines.
	    arg_parse,txt_flg[i],out, $		; Parse the i'th line.
	      keyabb=keyabb,keydef=keydef,keyful=keyful,/unsort
;	      keyabb=keyabb,keydef=keydef,keyful=keyful
	    ...
	    flg_tab = [flg_tab,intarr(n_elements(flg_nam0))+flg_tab0[i]]
	  endfor ; i
	  ...
	  uniq_tagval,flg_nam,flg_unam, $	; Unique indices.
	    /first,used=flg_usd
	endif else begin			; No flag lines.
	  n_flg_frm = 0				; Set no flag frames.
	  flg_tab = -1
	...
	if n_clr_frm gt 0 then begin			; If frame lines then
	  clr_frm_tab = tabnum[ind]			; Tab index for frame.
...
;	  keydef = ['16777215','16777215']		; White.
	...
	if n_clr0 gt 0 then begin		; If color lines then ...
	  clr_tab0 = tabnum[ind]		; Tab index for color.
	  txt_clr = t(ind)			; Extract them.
	  txt_clr = strmid(txt_clr,7)		; Trim off 'colors:'.

	  clr_tab = [0]				; Start color arrays.
...
;	      keyabb=keyabb,keydef=keydef,keyful=keyful
	    ...
	    clr_tab = [clr_tab,intarr(n_elements(clr_nam0))+clr_tab0[i]]
	  endfor ; i
	  ...
	  uniq_tagval,clr_nam,clr_unam, $	; Unique indices.
	    /first,used=clr_usd
	endif else begin			; No color lines.
	  n_clr_frm = 0				; Set no color frames.
	  clr_tab = -1
	...
	if n_lst_frm gt 0 then begin			; If frame lines then
	  lst_frm_tab = tabnum[ind]			; Tab index for frame.
	...
	if n_lst0 gt 0 then begin		; If list lines then ...
	  ...
	  for i=0, n_lst0-1 do begin		; Loop through list lines.
	    ...
	    lst_tab = [lst_tab,intarr(n_elements(lst_nam0))+lst_tab0[i]]
	  endfor ; i
	  ...
	  uniq_tagval,lst_nam,lst_unam, $	; Unique indices.
	    /first,used=lst_usd
	endif else begin			; No list lines.
	  n_lst_frm = 0				; Set no list frames.
	  lst_tab = -1
	...
	s = {n_ini:n_ini,             $  ; # INIT lines.
	     ...
	     n_tabs:n_tabs,           $  ; # of tabs.
	     tab_nam:tab_nam,         $  ; Tab names.

	     n_par_frm:n_par_frm,     $  ; # par_frames (1 minimum).
	     par_frm_tab:par_frm_tab, $  ; Tab index of each par frame.
	     ...
	     par_tab:par_tab,         $  ; Tab index of each parameter.
	     ...
	     par_frm:par_frm,         $
	     par_int:par_int,         $
	     par_usd:par_usd,         $  ; Unique indices.
;	     par_ind:par_ind,         $  ; Par indices.

	     n_flg_frm:n_flg_frm,     $  ; # flag frames (1 minimum).
	     flg_frm_tab:flg_frm_tab, $  ; Tab index of each flag frame.
