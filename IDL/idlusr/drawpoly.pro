...
;-------------------------------------------------------------
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   /MENU gives an exit menu: Done, Continue.'
	  print,'   /MAG use a cross-hair cursor with a mag window.'
	  print,'     This option is not very good yet.'
	  return
	endif
...
loop:	if keyword_set(mag) then begin
	  device, set_graph=3
	  crossi, ix, iy, device=dev, data=dat, normal=nor ,$
	    mag=4,size=500
	  device, set_graph=6
	endif else begin
	  cursor, ix, iy, 1, device=dev, data=dat, normal=nor
	endelse
	wait, .2
	...
	if !err eq 4 then begin
	  if keyword_set(menu) then begin
	    opt = xoption(['Done','Continue'])
	    if opt eq 1 then goto, loop
	  endif
