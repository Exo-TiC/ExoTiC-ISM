...
;-------------------------------------------------------------
	...
	function extremes_2d, z, flag, value=v, edges=edges, $
	  avoid=nedge, help=hlp
	...
	if (n_params(0) lt 2) or keyword_set(hlp) then begin
	  ...
	  print,'   AVOID=n_edge Number of pixels to ignore at edges (def=1).'
	...
	if n_elements(nedge) eq 0 then nedge=1  ; Ignore 1 edge pixel by def.
	...
	if not keyword_set(edges) then $
	  imgfrm, c, intarr(nedge)	; Drop edges (invalid).
