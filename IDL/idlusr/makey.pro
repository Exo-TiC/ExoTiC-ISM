...
;-------------------------------------------------------------
	...
	function makey, n, w, seed=seed0, lastseed=lseed, $
	  periodic=per,  min=mn, max=mx, help=hlp
	...
	if keyword_set(hlp) then begin
	  ...
	  print,' Keywords:'
	  print,'   MIN=mn Optional data minimum value (def=0.).'
	  print,'   MAX=mx Optional data maximum value (def=mn+1.).'
	...
	if n_elements(mx) eq 0 then mx=mn+1.
	if (mn ne 0.) or (mx ne 1.) then s=mn+s*(mx-mn)
