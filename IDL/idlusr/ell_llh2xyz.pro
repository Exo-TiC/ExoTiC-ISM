...
;-
;-------------------------------------------------------------
	pro ell_llh2xyz, lng1, lat1, htm, x, y, z, ellipsoid=in_ell, $
	  vectors=vectors, help=hlp
	...
	if (n_params(0) lt 6) or keyword_set(hlp) then begin
	  ...
	  print,'   VECTORS=v  Returned structure with [xp,yp,zp] = vectors'
	  print,'     from center to surface points at lng1, lat1, and'
	  print,'     [xh,yh,zh] = height vectors from surface point.  If called'
	  print,'     with htm=1 then [xh,yh,zh] will be unit surface normals.'
...
;	f1 = 298.257223563D0		; Reciprocal of flattening factor.
	...
	;  Returned vectors
	;--------------------------------------------------
	vectors = {xp:xp, yp:yp, zp:zp, xh:xh, yh:yh, zh:zh}
