...
;-------------------------------------------------------------
	...
	function tarclr, a1, a2, a3, set=set, hsv=hsv, help=hlp, $
	  ...
	  init=init, add=add, drop=drop, list=list, b24=b24, c24=c24
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   /C24 Always return 24 bit color.'
	  print,'     /B24 will not work for decomp=0 color mode.'
...
;	if !d.name eq 'Z' then decomp=0	; Z buffer is 8-bit color.
	if !d.name eq 'Z' then begin
	  device,get_pixel_depth=pixdep	; Check if 24-bit color.
	  if pixdep ne 24 then decomp=0	; Z buffer is 8-bit color.
	endif
	if !d.name eq 'X' then device,get_decomp=decomp
	if decomp eq 0 then flag24=0
	if keyword_set(c24) then flag24=1
	;------------------------------------------------
