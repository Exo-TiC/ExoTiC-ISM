...
;-
;-------------------------------------------------------------
	function bit_get, in, start, num0, reduce=reduce, help=hlp
 
	if (n_params(0) lt 2) or keyword_set(hlp) then begin
	  print,' Pick out specified bits from a value.'
	  print,' out = bit_get(in, s, n)'
	...
	if bits le 0 then begin
	  print,' Error in bit_get: must be an integer value.'
	  return,-1
	endif
	if n_elements(num0) eq 0 then num0=1
	num = num0 + 0
        if (start+num) gt bits then begin
          print,' Error in bit_get: '+$
...
else:   stop,' Stopped in bit_get: internal error.'
        endcase
