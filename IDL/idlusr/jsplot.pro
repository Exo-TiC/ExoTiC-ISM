...
;-------------------------------------------------------------
	...
	pro jsplot, t0, y0, help=hlp, charsize=csz, color=clr, $
	  ...
	  charthick=charthick, jsmajor=jsmajor, jsminor=jsminor, $
	  lshift=lshift, axes=axes
 
	common js_com, jsoff, last
 
	if ((n_params(0) lt 2) and (not keyword_set(axes))) or keyword_set(hlp) then begin
	  print,' Make a time series plot with time in Julian Seconds.'
	  print,' jsplot, js, y'
	  print,'   js = Time array in Julian Seconds.   in'
	  print,'        May also be an array of date/time strings.'
	  print,'   y  = Data to be plotted.             in'
	  print,' Keywords:'
	  print,'   /AXES used to replot the data and axes the same as they'
	  print,'     were done in the last jsplot. Call with no data:'
	  print,'       jsplot,/axes  OR jsplot,/axes, /nodata'
	  print,'     May also use color=clr to change color of axes (& data).'
	...
	if keyword_set(axes) then begin
	  if n_elements(last) eq 0 then begin
	    print,' Error in jsplot: Must make a plot using jsplot before'
	    print,'   calling jsplot,/axes'
	    return
	  endif
	  if n_elements(clr) eq 0 then clr=last.clr
	  plot, last.x, last.y, charsize=last.csz, color=clr, xstyle=5, $
	    xthick=last.xthick, ythick=last.ythick, charthick=last.charthick, $
	    xrange=last.xran,nodata=nodata,max_value=last.maxv, $
	    thick=last.thk, linestyle=last.lstyl, $
	    /noerase,_extra=last.extra
	  timeaxis, jd=last.jd, form=last.frm, nticks=last.ntics, $
	    title=last.xtitl, ticklen=last.ticklen, labeloffset=last.lbloff, $
	    charsize=last.csz, color=clr, thick=last.xthick, $
	    charthick=last.charthick,  lshift=last.lshift
	  return
	endif
 
	;-------  Allow date/time string for time  ---
	if datatype(t0) eq 'STR' then t=dt_tm_tojs(t0) else t=t0
...
;	if n_elements(cclr) eq 0 then cclr=clr
	...
	;------  Save last plot values (for /axes option)  ---
	if not keyword_set(over) then begin
	  last = {x:t-off, y:y, csz:csz, clr:clr, xthick:xthick, ythick:ythick,$
;		charthick:charthick, xran:tran-off, maxv:maxv, extra:extra, $
		charthick:charthick, xran:tran-off, maxv:maxv,  $
		jd:jd, frm:frm, ntics:ntics, xtitl:xtitl, ticklen:xtl*100, $
		lbloff:lbloff, lshift:lshift, lstyl:lstyl, thk:thk }
	  if n_elements(extra) ne 0 then begin
	    last=create_struct(last,'extra',extra)
	  endif else begin
	    last=create_struct(last,'extra','')
	  endelse
	endif
