...
;          precision Julian Seconds.  May be an array.
...
;       R. Sterner, 2008 Feb 20 --- Allowed time to be an array.
;       R. Sterner, 2009 Jun 03 --- Added /NOREFRACT.
;
...
;-------------------------------------------------------------
	pro sunpos, time, lng, lat, azi, alt, zone=zone, $
	  sublng=sunlng, sublat=sunlat, norefract=norefract, help=hlp
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'      precision Julian Seconds.  May be an array.'
	...
	if dt_tm_chk(time[0]) eq 0 then begin
	...
	if keyword_set(norefract) then begin
	  alt = 90-rr*!radeg
	endif else begin
