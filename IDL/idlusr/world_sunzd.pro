...
;         SUNLON=sunlng  Returned subsolar longitude.
;         SUNLAT=sunlat  Returned subsolar latitude.
;         /BYTE Output zd in a byte array, else float.
; OUTPUTS:
...
;       R. Sterner, 2012 Nov 20 --- Added /BYTE.
;       R. Sterner, 2012 Nov 20 --- ZONE was defaulting to local, now 0.
;
...
;-------------------------------------------------------------
	function world_sunzd, time0, res, zone=hrs, $
	  sunlng=sunlng, sunlat=sunlat, byte=byt, help=hlp
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   SUNLON=sunlng  Returned subsolar longitude.'
	  print,'   SUNLAT=sunlat  Returned subsolar latitude.'
          print,'   /BYTE Output zd in a byte array, else float.'
...
;	if n_elements(hrs) eq 0 then hrs=-gmt_offsec()/3600.	; Def=local.
	if n_elements(hrs) eq 0 then hrs=0.	; Def=0.
	...
	lat = transpose(lat)	  ; Want latitude to be a column.
	  ...
	  a = fltarr(nx,ny)	  ; Output zenith distances in floats.
        endelse
	...
	for i=0, nx-1 do begin
	  ...
	  cs = vx*sx + vz*sz                            ; Lat vector dot sun v.
	    ...
	    a[i,0] = acos(cs)*!radeg                    ; Zenith Dist.
          endelse
	endfor
