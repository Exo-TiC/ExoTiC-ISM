...
;-------------------------------------------------------------
	...
	;--------  Defaults  ----------------------------------
	;----  Common:
	if n_elements(text0) eq 0 then text0 = 'Text'	; Default text.
	if n_elements(thk0) eq 0 then thk0 = [3,1]	; Default thicknesses.
	if n_elements(fnt0) eq 0 then fnt0 = '!3'	; Default font.
	if n_elements(a0) eq 0 then begin		; Default left point.
	  a0 = [round(.3*!d.x_size),round(.6*!d.y_size)]
	endif
	if n_elements(b0) eq 0 then begin		; Default right point.
	  b0 = [round(.6*!d.x_size),round(.6*!d.y_size)]
	endif
	if n_elements(clr0) eq 0 then begin		; Default colors.
	  tvlct,r,g,b,/get
