...
;-------------------------------------------------------------
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,' Keywords:'
	  print,'   /IGNORE_CASE means ignore case for normal or reverse'
	  print,'     lookup.'
	...
	;-------------------------------------------------
	if n_elements(key) ne 0 then begin	; Key was given.
	  if keyword_set(igcase) then begin	; Search ignoring case.
	    w = where(strupcase(arr) eq strupcase(key),cnt)
	  endif else begin			; Case-sensitive search.
	    w = where(arr eq key, cnt)
	  endelse
	  if cnt eq 0 then begin		; Not there, error.
	    if not keyword_set(quiet) then begin
	      print,' Error in aarr: Given key not found'
...
;	    return, arr				; No change.
	...
	if n_elements(val) ne 0 then begin
	  if keyword_set(igcase) then begin	; Search ignoring case.
	    w = where(strupcase(arr) eq strupcase(val),cnt)
	  endif else begin			; Case-sensitive search.
	    w = where(arr eq val, cnt)		; Find it in array.
	  endelse
	  if cnt eq 0 then begin		; Not there, error.
