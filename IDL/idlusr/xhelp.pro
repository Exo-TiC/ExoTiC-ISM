...
;=============================================================
	...
	pro xhelp, txt, title=title, lines=lines, exit_text=texit, $
	  ...
	  t2label=t2lab, bottom=bot, command=cmd, comlabel=cmd_txt
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'     in the array txt2.  txt2 must be given for this button to appear.'
          print,'   COMMAND=cmd Optional command line command to execute.'
          print,'     This command will be spawned from IDL.  If given and not null'
          print,'     a button will be added to the widget.'
          print,'   COMLABEL=comlabel Label for command button (default is "Command").'
	    ...
	    id_t2=widget_button(b1,value=t2lab,uval='TEXT2')
          ...
          if n_elements(cmd) gt 0 then begin
            if cmd ne '' then begin
              if n_elements(cmd_txt) eq 0 then cmd_txt='Command'
              id_sp = widget_button(b1,value=cmd_txt,uval='CMD')
            endif
          endif
	endif else begin
	  txt2 = ''
...
;	tid = widget_text(top,value=txt,xsize=nx,ysize=ny,scroll=scroll)
	...
	s = {txt1:txt,txt2:txt2,state:0,xsize:xsize,lines:lines, $
	     file:fsave,scroll0:scroll0,tid:tid,cmd:cmd}
