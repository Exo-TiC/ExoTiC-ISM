...
;	R. Sterner, 2003 Oct 22 --- Added /ynoz.  Also fixed xprint call.
;	R. Sterner, 2003 Oct 22 --- Added eqtxt. Also plot titles.
;
...
;-------------------------------------------------------------
	...
	ttl = ''
	read,' Enter plot title: ',ttl
	ttx = ''
	read,' Enter X axis title: ',ttx
	tty = ''
	read,' Enter Y axis title: ',tty
	  ...
	  if (ptype(0) eq 0) and (ptype(1) eq 0) then plot, $
	    x(lo:hi), y(lo:hi), psym=psym, pos=pos, symsize=.5,/ynoz, $
	    title=ttl, xtitle=ttx, ytitle=tty
	  ...
	  xprint, /init, .1, .55, charsize=.7, dy=1.5, /norm
