	...
	;  R. Sterner, 2007 Sep 20 --- Added a bit to the help text.
	;  R. Sterner, 2007 Sep 25 --- Fixed help text for GET method.
	;  R. Sterner, 2009 May 28 --- Fixed multipoly.  Was setting clipping
	;    window outside zwindow so using the wrong screen size.
	;  R. Sterner, 2011 Sep 03 --- Skip img_disp if 'Z' buffer.
	;
	;  Methods:
	;  ------ External --------
	;  pro img_aa::help
	...
	print,'     The procedure can take inputs, positional or keyword,'
	print,'     but outputs will not be passed back.'
	...
	print,'   Or use the /MATCHPOS keyword.'
	...
	print,'   This method is for plotting on the image in a single'
	print,'   given color only.  Image related commands will not work.'
	print,' '
	return
	end
	...
	zwindow,/bits8,xs=self.nx,ys=self.ny	; Z buffer window.
	case flag of
1:	begin
	  ix = round(x)
	  iy = round(y)
...
3:	begin
	...
	zwindow,/bits8,xs=self.nx3,ys=self.ny3	; Z buffer window (3x bigger).
	;-----------------------------------------------
...
;X	  c_win = 3*[t1(0),t2(0),t1(1),t2(1)]	; commands below.
	  c_win = [t1(0),t2(0),t1(1),t2(1)]	; commands below.
...
;X	zwindow,xs=self.nx3,ys=self.ny3	; Z buffer window (3x bigger).
;X	zwindow,/bits8,xs=self.nx3,ys=self.ny3	; Z buffer window (3x bigger).
