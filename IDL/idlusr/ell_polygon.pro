...
;-------------------------------------------------------------
	...
	for i1=0L, n-1 do begin
	  ...
	  ;--------------------------------------------------------
'G':	  begin
	    if (lng1 eq lng2) and (lat1 eq lat2) then begin ; Deal with 0 dist case.
	      d = 0.
	    endif else begin
...
;	      py = [py,lat1,lat2]
	      ...
	      num = ceil(d/stp)+1			; Keep step sz LE step.
;	      dd = maken(0D0,d,100)			; Distances.
	      dd = maken(0D0,d,num)			; Distances.
	      ell_rb2ll, lng1,lat1, dd, azi, px1, py1	; Points on geodesic.
;	      px = [px,px1]
;	      py = [py,py1]
	      px = [px,px1[0:num-2]]			; Don't include last val in
	      py = [py,py1[0:num-2]]			; dd (gets added next step).
	    endelse
	  end
	  ;-----------  Loxodromic side  -----------------
	  ;--------------------------------------------------------
