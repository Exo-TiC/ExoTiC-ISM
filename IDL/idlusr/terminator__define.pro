	...
	;	R. Sterner, 2009 Apr 10 --- Added calls to empty.
	;	R. Sterner, 2012 Nov 28 --- Upgraded help text.
	;	R. Sterner, 2012 Nov 28 --- Added a help method.
	;===========================================================
	...
	if (n_params(0) eq 0) or keyword_set(hlp) then begin
	  ...
	  print,'   /XMODE for XOR.'
	  print,'   OFFSET=off Terminator offset in degrees from subsun pt (def=90).'
...
;
;  Terminator Object
;    Plots the terminator and sub-sun position on current map given GMT time.
;
;  To create a terminator object named sun:
;    sun = obj_new('terminator')
;    Keywords:
;      ALT=km  Optional satellite altitude in km.  If given will
;        compute offset for terminator at satellite altitude.
;    In the listing of methods below sun will be used as the object name.
;
;  Methods:
;
;    Overall help
...
;      No args.
;
;    Plot terminator and subsun position.
;    sun.draw, time
;      time = Time in GMT, JS or date/time string.
;        Could use dt_tm_tojs(systime())+gmt_offsec()
;    Keywords:
;      COLOR=clr, LINESTYLE=sty, THICKNESS=thk,
;      CHARSIZE=csz Character size.
;      NOSUN=ns  No sun flag.
;      /XMODE for XOR.
;      OFFSET=off Terminator offset in degrees from subsun pt (def=90).
;      ALT=km  Satellite altitude in km instead of OFFSET.
;        Will compute offset for terminator at satellite altitude.
;      /HELP show help for this method.
;
;    Set parameters for the terminator object
;    sun.set
;      All args are keywords:
;    Keywords:
;      FLAG=flag Plot flag, set to 0 for a new plot if needed.
;      NOSUN=ns  Sun no plot flag: 0=plot sun, 1=do not plot sun.
;      COLOR=clr Plot color (def=yellow).
;      LINESTYLE=styl Plot line style (def=0).
;      THICKNESS=thk Plot thickness (def=1).
...
;      /HELP show help for this method.
;
;    List current status of the terminator object
;    sun.list
;      No args.
;
