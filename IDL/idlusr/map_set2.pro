...
;       R. Sterner, 2011 May 10 --- Handled flag values better.
;
...
;x	flag_noborder = c ne 0
        if c gt 0 then v=extra.(w[0]) else v=0
        flag_noborder = byte(v)
...
;X	flag_iso = c ne 0
	...
	;--- CLIP=0:  (Default is to clip)
	w = where(strmid(tnames,0,2) eq 'CL',c)
;X	flag_clip = c ne 0
        if c gt 0 then v=extra.(w[0]) else v=1 ; 1 means clip.
        flag_clip = byte(v eq 0)
