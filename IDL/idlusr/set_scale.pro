...
;-------------------------------------------------------------
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'     the display.'
	  print,'   /NOSET do not actually set scaling.  This is useful with'
          print,'     the IMAGE=img option to list or get the scaling info.'
...
;	s = string(t(10:*))
	...
	if not keyword_set(noset) then begin
	  plot,[x1,x2],[y1,y2],pos=[ix1,iy1,ix2,iy2],/dev,$
            xrange=[x1,x2], yrange=[y1,y2], $   ; Added RES 2010Mar10.
	...
	if keyword_set(list) then begin
	  ...
	  print,' '
        ...
        ;---  Pack info in structure to return  ---
	if arg_present(out) then begin
	  out = {ix1:ix1,ix2:ix2,iy1:iy1,iy2:iy2,nx:nx,ny:ny, $
		x1:x1,x2:x2,y1:y1,y2:y2,nx_img:nx_img,ny_img:ny_img}
	endif

        ;---  Deal with map_set  ---
        if (keyword_set(mset) and (not keyword_set(noset))) then begin
          pos = [ix1,iy1,ix2,iy2]/float([nx_img,ny_img,nx_img,ny_img])
          lim = [y1,x1,y2,x2]
          map_set2,0,0,lim=lim,pos=pos,/noerase,/noborder
          map_put_scale
        endif
