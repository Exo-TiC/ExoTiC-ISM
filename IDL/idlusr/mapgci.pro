...
;-------------------------------------------------------------
;  TO DO:
;	[ ] Add optional planet radius input. Only applies to Earth now.
;	[ ] Allow interactive radius pick.  Maybe use mapgci but
;	    list radius as it is adjusted.
;-------------------------------------------------------------
	pro mapgci, a, b, thickness=thk, linestyle=sty, help=hlp, $
	  nohelp=nohelp, units=units, notes=notes_txt, $
	  start=start, title=ttl
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   START=n  Initial active point (1=A def, 2=B).'
	...
	flag = 1		; By default active point is A (point 1).
	if n_elements(start) gt 0 then flag=start  ; Initial point given.
...
'ra':   begin
	...
	;---  If given initial line is bad try to find one.
	if finite(irr,/nan) eq 1 then begin		; Try plan B.
	  x50 = !d.x_size/2	; Window center.
	  y50 = !d.y_size/2
	  fct = 4		; Try 1/4 way to center to start.
	  try = 1		; Only try a few times.
planb:
	  ddx = !d.x_size/fct	; Part way to center.
	  ddx = !d.y_size/fct
	  x25 = x50 - ddx
	  y25 = y50 - ddy
	  tmp = convert_coord([x25,x50],[y25,y50],/dev,/to_data)
	  w = where(finite(tmp,/nan),cnt)	; Any bad points?
	  if cnt ne 0 then begin		; Yes.
	    if try ge 4 then begin		; More trys left?
	      print,' Error in map_gci: cannot find good initial points.'
	      return
	    endif
	    fct = fct*2
	    try += 1
