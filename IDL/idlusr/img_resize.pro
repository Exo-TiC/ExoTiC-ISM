...
;-------------------------------------------------------------
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'   correct image planes.'
          ...
          print,'   A 1-D color image has dimensions n,1,3 in any order.'
          print,'   If the 1 is last it may be lost, add it back using reform.'
	  return, ''
	endif
...
2:	begin
	...
	if sflag eq 1 then begin
	  ...
	  img2 = img_smooth(img,sm)
          ...
          dm = size(img,/dimensions)
	  img2 = reform(img,dm)         ; Avoid dropping any trailing 1.
	endelse
	...
	if tr eq 0 then begin
	  if keyword_set(rbin) then begin
	    return, rebin(img2,nx,ny,_extra=extra)
            ...
            if ny0 eq 1 then img2=reform(img2,nx0,ny0)    ; Needed if ny0 is 1.
	...
	if keyword_set(rbin) then begin
	  ...
	  b = rebin(b,nx,ny,_extra=extra)
          ...
          if (nx0 eq 1) or (ny0 eq 1) then begin        ; Needed if ny0 is 1.
            r=reform(r,nx0,ny0)
            g=reform(g,nx0,ny0)
            b=reform(b,nx0,ny0)
          endif
