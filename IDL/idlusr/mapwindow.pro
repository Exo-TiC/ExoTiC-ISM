...
;-------------------------------------------------------------
	pro mapwindow, help=hlp, error=err, quiet=quiet, $
	  ...
	  outline=outline, color=clr, thickness=thk, relaxed=relaxed
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   ERROR=err Error flag: 0=OK, 1=last plot not map,'
          print,'      2=no last plot.'
	  ...
	  print,'   THICKNESS=thk  Outline thickness (def=!p.thick).'
          print,'   /RELAXED Do not require strict map, allow xy plot.'
	...
	if !x.type ne 3 then begin
	  ...
	  err = 1
	  if not keyword_set(relaxed) then return
	endif
	...
	;------  Check if some kind of plot  -----------------
        if nx eq 0 then begin
	  if not keyword_set(quiet) then $
            print,' Error in mapwindow: No plot.'
	  err = 2
          return
        endif
