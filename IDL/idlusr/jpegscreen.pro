...
; CATEGORY:
; CALLING SEQUENCE:
;       jpegscreen, [file], quality = q, quiet = quiet 
; INPUTS:
...
;         /quiet Don't echo output file name to screen. 
; OUTPUTS:
; COMMON BLOCKS:
; NOTES:
...
;       Mary Ruth Keller, 2011 NOV 15 --- /quiet added. 
;       R. Sterner, 2011 Nov 16 --- Converted arrays from () to [].
;
...
;-
;-------------------------------------------------------------
	pro jpegscreen, file, quality = q, quiet = quiet, help = hlp
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'     Low values give better compression but poorer quality.'
          print,"     /quiet  Don't echo output file name to screen."
	  print,' Notes: Prompts for file if called with no args.'
	  print,'   Also works for z-buffer (but only as pseudo color).'
	  return
	endif
