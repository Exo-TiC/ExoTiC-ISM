...
;-------------------------------------------------------------
	...
	;--------  Find values  -----------------
;x	if !x.crange[0] eq tx1 then tx1=tx1+dx
;x	if !x.crange[1] eq tx2 then tx2=tx2-dx
;x	if !y.crange[0] eq ty1 then ty1=ty1+dy
;x	if !y.crange[1] eq ty2 then ty2=ty2-dy
	xtol = dx/10000.	; Avoid roundoff error.
	ytol = dy/10000.
	if abs(!x.crange[0]-tx1) lt xtol then tx1=tx1+dx
	if abs(!x.crange[1]-tx2) lt xtol then tx2=tx2-dx
	if abs(!y.crange[0]-ty1) lt ytol then ty1=ty1+dy
	if abs(!y.crange[1]-ty2) lt ytol then ty2=ty2-dy
