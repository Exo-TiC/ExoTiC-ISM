...
;==============================================================
	...
	pro xmess, txt, wid=top, nowait=nowait, oktext=oktxt, $
	  wait=wait, help=hlp, xoffset=xoff, yoffset=yoff,left=left
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'   /LEFT means left align text, else center it.'
	  return
	endif
        ...
        if !d.name ne 'X' then begin
          txt1 = txtpad(txt)
          tm = '###   '+txt1+'   ###'
          ts = '###   '+spc(strlen(txt1[0]))+'   ###'
          tf = spc(strlen(tm[0]),char='#')
          tt = ' '+[tf,tf,ts,tm,ts,tf,tf]
          more,tt,lines=999999
          return
        endif
	...
	for i = 0, n_elements(txt)-1 do $
	  id = widget_label(top, val=txt[i],align_left=left)
	id = widget_label(top, val=' ')
