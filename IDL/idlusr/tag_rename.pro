...
;-
;-------------------------------------------------------------
	function tag_rename, s1, list0, error=err, help=hlp
 
	if (n_params(0) lt 2) or keyword_set(hlp) then begin
	  print,' Rename tags in a structure.'
	  print,' s2 = tag_rename(s1,list)'
	...
	if (flag lt 1) or (flag gt 2) then begin
	  print,' Error in tag_rename: The given list must'
	...
	if ntags ne nlist then begin
	  print,' Error in tag_rename: Must give a list with as many'
...
2:	begin					; Both OLD and NEW given.
	  ...
	  if cnt eq 0 then begin
	    print,' Error in tag_rename: Old tag name not found: '+old[0]
	    err = 1
	    return, ''
	  endif
	  j = w[0]
	  ...
	  for i=1,ntags-1 do begin		; Add each item.
	    ...
	    if cnt eq 0 then begin
	      print,' Error in tag_rename: Old tag name not found: '+old[i]
	      err = 1
	      return, ''
	    endif
	    j = w[0]
