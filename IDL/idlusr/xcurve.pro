...
;           possible.
;         XCOLOR=xclr  Set plot color (def=white).  A light gray
;           about 200 works better if image has gray of 128
;           which will hide the XOR lines.  XCOLOR=tarclr(200).
; OUTPUTS:
...
;       R. Sterner, 2012 Nov 15 --- Added XCOLOR=xclr.
;
...
;-----------------------------------------------------------------
	...
	pro xplot, x1,x2,y1,y2,device=dev,data=dat,normal=nor, xcolor=xclr
	...
	device, set_graph=6			; XOR mode.
        plots,[x1,x2],[y1,y2],data=dat,dev=dev,norm=nor,color=xclr
	empty
...
;=================================================================
	...
	pro xcurve, xa, ya, device=dev, data=data,normal=norm, $
	  mag=mag0, size=msize0, step_min=step, autocenter=autocenter, $
          xcolor=xclr, help=hlp
	...
	if keyword_set(hlp) then begin
	  ...
	  print,'     possible.'
          print,'   XCOLOR=xclr  Set plot color (def=white).  A light gray'
          print,'     about 200 works better if image has gray of 128'
          print,'     which will hide the XOR lines.  XCOLOR=tarclr(200).'
	  return
	endif
	...
	;------  Find color depth  ---------------
        ...
        if n_elements(xclr) eq 0 then xclr=!p.color
	...
	while !mouse.button ne 4 do begin
	  ...
	  ;-------  Middle button: Drop point.  ---------
		...
		data=data,dev=dev,norm=norm,xcolor=xclr
