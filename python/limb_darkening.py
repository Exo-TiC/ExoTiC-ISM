...
def limb_fit_3D_choose(grating, widek, wsdata, M_H, Teff, logg, dirsen, direc):
    ...
    ----------
    grating: string 
        grating to use ('G430L','G750L','WFC3','R500B','R500R')
    wsdata: array 
        data wavelegnth solution
    widek:  array
        index array of wsdata, indicating bin of pixels to use 
    Teff: float
    ...
    cp1, cp2, cp3, cp4: float
        three-parameter limb darkening coefficients
    c1, c2, c3, c4: float
...
def nonlinear_limb_darkening(x, c0=0.0, c1=0.0, c2=0.0, c3=0.0):
    model =  (1. - (c0*(1. - x**(1./2)) + c1*(1. - x**(2./2)) + c2*(1. - x**(3./2)) + c3*(1. - x**(4./2))))
    return model
...
def main():
    ...
    return limb_fit_3D_choose(grating, widek, wsdata, FeH, Teff, logg, dirsen, direc)
